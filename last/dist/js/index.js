...
/***/ function(module, exports, __webpack_require__) {
	...
	var Site = __webpack_require__(3);
	...
	function bandEvent() {    
	    ...
	    $('.pro').on('change',function() {
	        ...
	        if(pro == '选择省份') {
	            ...
	            for(var i in address[pro]) {
	                $('.city').append('<option>' + address[pro][i] + '</option>');
	            } 
	        }       
	    });
	...
	function confirmInfo(){
	    Site.loading(true);
	    $.getJSON(window.location.origin+"/default/index/review",{
	        outlet_id: _outlet_id,
	        status: _status
	    }).then(function(res){
	        if(res.success){
	            $('.cont4').show().siblings('.cont').hide();
	        }else{
	            Site.alert(res.errorMsg);
	        }
	    },function(){
	        Site.alert('网络繁忙，请稍后重试！');
	    }).always(function(){
	        Site.loading(false);
	    });
	}

	function uploadTicket(_file){
	    Site.loading(true); 
	    var formData = new FormData();
	    formData.append('bill_pic', _file);    
	    formData.append('openid', _openid);    
	    $.ajax({
	        url: window.location.origin + '/default/index/bill',
	        type: 'POST',
	        dataType: 'json',
	        data: formData,
	        contentType: false,
	...
	function weixinShare(){
	    $.getJSON('/weixin/index/get-jssdk-info?jsonpcallback=?', {
	    ...
	    wx.ready(function () {        
	        var title = ' ';
	        var desc = '小长假出行攻略！首单满60减10、送旅行分装瓶、抽DIY盆栽等4重福利等着你！';
	        var imgUrl = window.location.origin + '/html/paylottery/images/share.jpg';
	        var link = window.location.origin + '/html/paylottery/list.html';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [
	...
	], __WEBPACK_AMD_DEFINE_RESULT__ = function($, _, MsgBox) {
	    /**
	     * @class Site
	     */
	    function Site() {}

	    Site.onError = function(cb) {
	        Site.loading(false);
	        Site.alert('网络繁忙，请稍候重试!', cb);
	    };
	    ...
	    Site.loading = function(isVisible) {
	        _loading.push(isVisible);

	        clearTimeout(_timeout);
	        _timeout = setTimeout(function() {
	            var selector = '.loading';
	            if (0 == $(selector).size()) {
	                $('body').append('<div class="loading loading-x2 loading-overlay loading-hide"></div>');
	            }
	            if (_loading.pop()) {
	                $(selector).show();
	            } else {
	     ...
	     * @returns 返回渲染后的字符串
	     */
	    Site.render = function(template, data) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function(require, exports) {
	    var $ = __webpack_require__(1);
	    var ImgLoader=function(elm,opt,arr){
	        this.preloadArr= arr || [];
	        this.foundUrls=[];
	        this.total=0;
	        this.cur=0;
	    ...
	    ImgLoader.prototype.init = function() {
	        
	        this.findImageInElement(this.elm);
	        if(this.options.deepSearch){
	            var elements =$(this.elm).find("*:not(script)");
	    ...
	    ImgLoader.prototype.createImg=function(){
	        var self=this;
	        this.total=this.preloadArr.length;
	        this.cur=0;
	        for(var i=0;i<this.preloadArr.length;i++){
	            var imgObj=new Image();
	            imgObj.onload=function(){
	                self.cur++;
	               // console.log(self.cur+"   "+self.total)
	                if(self.cur>=self.total){
	                    self.options.onComplete();
	                }else{
	                    var per=100*self.cur/self.total;
	                  
	                    self.options.onProgress(per);
	                }
	            }
	            //console.log(self.preloadArr[i])
	            imgObj.src=self.preloadArr[i];
	        }
	    }
	    ImgLoader.prototype.findImageInElement=function(element) {
	        var url = "";
	        var obj = $(element);
	        var type = "normal";
	        if(typeof obj.css('filter')=='string'&&obj.css('filter').indexOf('DXImageTransform')!=-1){
	            url=obj.css('filter').match(/images.*.png|jpg|gif/)[0];
	        }else if (obj.css("background-image") != "none"&&/images.*.png|jpg|gif/.test(obj.css("background-image"))) {
	        ...
	        if (!hasGradient(url)) {
	            ...
	            for (var i = 0; i < urls.length; i++) {
	                if (validUrl(urls[i]) && urlIsNew(this.foundUrls,urls[i])) {        
	                    var extra = "";
	                    if (isIE() || isOpera()){
	                        //filthy always no cache for IE, sorry peeps!
	                        extra = "?rand=" + Math.random();
	                        //add to preloader
	                        this.preloadArr.push(urls[i] + extra);
	                    } else {
	                        if (type == "background") {
	                            //add to preloader
	                            this.preloadArr.push(urls[i] + extra);
	                        } else {
	                            this.preloadArr.push(url);
	                        }
	                    }
	   ...
	   $.fn.preloadImg = function(opt, arr){
	        return this.each(function(){
	            (new ImgLoader(this, opt, arr));
	        });
	    }
	    $.preloadImgArr=function(imgArr,opt){
	        new ImgLoader(null, opt,imgArr);
	    }
	}.call(exports, __webpack_require__, exports, module), __WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function(require,exports,module){
	    ...
	    exports.checkTel=function(url){
	        var reg =/^(13|14|15|17|18)\d{9}|\d{3,4}\-\d{7,8}$/;
	        if (reg.test(url)) {
	            return true;
	        }
	        return false;
